# ---- app_james.R bootstrap: ensure app-only Suggests are available -----------
...
## Retrieves BDS dossier data uploaded to JAMES via james::request_tab10().
## The URL carries ?session=<key>; the session's .val (list(psn=, xyz=)) is
## fetched from JAMES's /{session}/rda endpoint (a binary .RData payload,
## avoiding a JSON round-trip of data.frame structure). See
## /Users/buurensv/.claude/plans/resilient-wiggling-clarke.md for the full
## dataflow this implements.
fetch_james_session <- function(session_key, james_base = Sys.getenv("JAMES_BASE_URL", "http://james")) {
  tmp <- tempfile(fileext = ".RData")
  on.exit(unlink(tmp), add = TRUE)
  resp <- httr2::request(james_base) |>
    httr2::req_url_path_append(session_key, "rda") |>
    httr2::req_perform()
...
server <- function(input, output, session) {
  ...
  current.target <- reactive({
    ...
    ses <- query[['session']]
    if (is.null(ses) || !nzchar(ses)) return(NULL)

    child_data <- tryCatch(
      fetch_james_session(ses),
      error = function(e) {
        showNotification(
          paste("Kan dossier niet ophalen van JAMES:", conditionMessage(e)),
          type = "error"
        )
        NULL
      }
    )
    if (is.null(child_data) || !all(c("psn", "xyz") %in% names(child_data))) {
      if (!is.null(child_data)) {
        showNotification("Ontvangen dossierdata heeft onverwacht formaat.", type = "error")
      }
      return(NULL)
    }

    collect_predictors(child_data, purpose = "tab10")
  })
...
# ---- Models ----
  ...
  father_country_beta <- reactive(
    ...
    beta_lookup(temp_father_country(), "Geboorteland moeder", "overweight-4y")
  )
  fa_beta <- reactive({
